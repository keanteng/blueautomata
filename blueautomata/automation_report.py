        ...
        def report_summary(dataframe, staff_data, checklist, key):
            ...
            if key == 1:
                ...
                if 'STATSMART CUBE' in df.columns:
                    ...
                    df = df.drop_duplicates(subset = ['User ID','Cube', 'System1'], keep = 'first')
                ...
                for j in range(0,len(self.name_key)):
                    ...
                    if checker in folder_list[k]: 
                        ...
                        if self.name_code[j] == 1:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 1)
                            dfs.append(df)
                        elif self.name_code[j] == 2:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 2)
                            dfs.append(df)
                        elif self.name_code[j] == 3:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 3)
                            dfs.append(df)
                        elif self.name_code[j] == 4:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 4)
                            dfs.append(df)
                        elif self.name_code[j] == 5:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 5)
                            dfs.append(df)
                        elif self.name_code[j] == 6:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 6)
                            dfs.append(df)
                        else:
                            df = report_summary(excelFile, self.staff_data, self.checklist, key = 10)
